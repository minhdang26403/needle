...
##############################################################################
### forward passes TESTS
def test_power_scalar_forward() -> None:
    np.testing.assert_allclose(
        (Tensor([[0.5, 2.0, 3.0]]) ** 2).numpy(),
        np.array([[0.25, 4.0, 9.0]]),
    )


def test_divide_forward() -> None:
    np.testing.assert_allclose(
        (
...
def test_divide_scalar_forward() -> None:
    np.testing.assert_allclose(
        (Tensor([[1.7, 1.45]]) / 12).numpy(),
        np.array([[0.141666666667, 0.120833333333]]),
    )


def test_matmul_forward() -> None:
    np.testing.assert_allclose(
        (
...
def test_summation_forward() -> None:
    np.testing.assert_allclose(
        Tensor(
            [
...
def test_broadcast_to_forward() -> None:
    np.testing.assert_allclose(
...
def test_negate_forward() -> None:
    np.testing.assert_allclose(
        (-Tensor([[1.45, 0.55]])).numpy(), np.array([[-1.45, -0.55]])
    )


def test_transpose_forward() -> None:
    np.testing.assert_allclose(
        Tensor([[[1.95]], [[2.7]], [[3.75]]]).transpose(axes=(1, 2)).numpy(),
        np.array([[[1.95]], [[2.7]], [[3.75]]]),
    )
    np.testing.assert_allclose(
        Tensor([[[[0.95]]], [[[2.55]]], [[[0.45]]]]).transpose(axes=(2, 3)).numpy(),
    ...
    np.testing.assert_allclose(
        Tensor([[[2.45]], [[3.5]], [[0.9]]]).transpose(axes=(0, 1)).numpy(),
        np.array([[[2.45], [3.5], [0.9]]]),
    )
    np.testing.assert_allclose(
        Tensor([[4.4, 2.05], [1.85, 2.25], [0.15, 1.4]]).transpose().numpy(),
...
def test_ewisepow_forward() -> None:
    np.testing.assert_allclose(
        (Tensor([[1.0, 2.0, 3.0]]) ** Tensor([[0, 0, 2]])).numpy(),
        np.array([[1.0, 1.0, 9.0]]),
    )


##############################################################################
### backward passes TESTS
...
def gradient_check(
    ...
    **kwargs: Any,
    ...
    if not backward:
        ...
        computed_grads = [
            x.numpy() for x in out.op.gradient_as_tuple(Tensor(np.ones(out.shape)), out)
        ]
    else:
...
def test_power_scalar_backward() -> None:
    gradient_check(
        lambda a, scalar: a**scalar,
        Tensor(np.random.randn(5, 4)),
        scalar=np.random.randint(1),
    )


def test_divide_backward() -> None:
    gradient_check(
        lambda a, b: a / b,
        Tensor(np.random.randn(5, 4)),
...
def test_divide_scalar_backward() -> None:
    gradient_check(
        lambda a, scalar: a / scalar,
        Tensor(np.random.randn(5, 4)),
...
def test_matmul_simple_backward() -> None:
    gradient_check(
        lambda a, b: a @ b, Tensor(np.random.randn(5, 4)), Tensor(np.random.randn(4, 5))
    )


def test_matmul_batched_backward() -> None:
    def matmul(a: Tensor, b: Tensor) -> Tensor:
        return a @ b

    gradient_check(
        matmul, Tensor(np.random.randn(6, 6, 5, 4)), Tensor(np.random.randn(6, 6, 4, 3))
    )
    gradient_check(
        matmul, Tensor(np.random.randn(6, 6, 5, 4)), Tensor(np.random.randn(4, 3))
    )
    gradient_check(
        matmul, Tensor(np.random.randn(5, 4)), Tensor(np.random.randn(6, 6, 4, 3))
    )
...
def test_broadcast_to_backward() -> None:
    ...
    gradient_check(broadcast_to, Tensor(np.random.randn(5, 4, 1)), shape=(5, 4, 3))


def test_summation_backward() -> None:
    def summation(a: Tensor, axes: tuple[int, ...]) -> Tensor:
        return a.sum(axes)
...
def test_log_backward() -> None:
    ...
    gradient_check(log, Tensor(1 + np.random.rand(5, 4)))
...
def test_exp_backward() -> None:
    ...
    gradient_check(exp, Tensor(1 + np.random.rand(5, 4)))


def test_ewisepow_backward() -> None:
    def ewisepow(a: Tensor, b: Tensor) -> Tensor:
        return a**b

    gradient_check(ewisepow, Tensor([[1.0, 2.0, 3.0]]), Tensor(np.ones((1, 3)) * 2))
